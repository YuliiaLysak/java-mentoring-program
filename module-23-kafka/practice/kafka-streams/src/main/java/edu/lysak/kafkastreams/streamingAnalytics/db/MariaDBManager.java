package edu.lysak.kafkastreams.streamingAnalytics.db;
...
public class MariaDBManager implements Runnable, Serializable {
    ...
    public void setUp() {
        ...
        try {
            connection = DriverManager.getConnection(url, "streaming", "streaming");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void run() {
        try {
            ...
            String idQuery = "SELECT IFNULL(MAX(ID),0) as LATEST_ID "
                    + "FROM order_summary";
            ResultSet rsLatest = connection.createStatement().executeQuery(idQuery);
